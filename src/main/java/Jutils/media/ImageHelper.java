package Jutils.media;


import Jutils.io.FileHelper;
...
//     boolean ret = false;
     ...
     * @throws Exception
     */
    public static boolean compress(InputStream in, File saveFile, int rate) {
        if(null == in ||null == saveFile ||rate < 1){// 检查参数有效性
            return false;
        }
        ...
        // 原图的大小
        int width = srcImage.getWidth() / rate;
        int height = srcImage.getHeight() / rate;
...
//        File saveFile = new File("d:\\ioc\\files\\aaa2.jpg");
        ...
        FileHelper fileHelper = FileHelper.getFileHelper();

        File saveFile = fileHelper.saveFileWithAbsolutePath("d:\\ioc\\files\\aaa2.jpg", "");

        String photoPath = "D:\\M\\tomcat8.0\\webapps\\ROOT\\mp3\\因为一个人.jpg";
        in = new FileInputStream(new File(photoPath));
        BufferedImage srcImage = null;
...
//        compressTest();
    ...
    private static void compressTest() throws IOException {
        /**
        ...
        String photoPath = "D:\\M\\tomcat8.0\\webapps\\ROOT\\mp3\\因为一个人.jpg";
        File saveFile = new File(photoPath+".jpg");
        try {
            //原图片的路径
            in = new FileInputStream(new File(photoPath));
